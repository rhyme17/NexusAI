...
from ..core.config import get_postgres_dsn, get_seed_file, is_seed_apply_if_empty, is_seed_enabled
...
class PostgresStore(StoreContract):
    """PostgreSQL-backed store using JSONB payloads for stage-1 migration."""

    def __init__(self, *, dsn: str | None = None) -> None:
        self._lock = Lock()
        self._dsn = dsn or get_postgres_dsn()
        if not self._dsn:
            raise ValueError("NEXUSAI_POSTGRES_DSN is required when NEXUSAI_STORAGE_BACKEND=postgres")
    ...
    def _upsert_agent(self, agent: Agent) -> None:
        payload = self._serialize_payload(agent.model_dump(mode="json"))
        with self._connection.cursor() as cursor:
            cursor.execute(
                """
    ...
    def _upsert_task(self, task: Task) -> None:
        payload = self._serialize_payload(task.model_dump(mode="json"))
        with self._connection.cursor() as cursor:
            cursor.execute(
                """
    ...
    def _load_agent(self, agent_id: str) -> Agent | None:
        with self._connection.cursor() as cursor:
            cursor.execute("SELECT payload_json FROM agents WHERE agent_id = %s", (agent_id,))
            row = cursor.fetchone()
        if not row:
            return None
    ...
    def _load_task(self, task_id: str) -> Task | None:
        with self._connection.cursor() as cursor:
            cursor.execute("SELECT payload_json FROM tasks WHERE task_id = %s", (task_id,))
            row = cursor.fetchone()
        if not row:
            return None
        payload = self._deserialize_payload(row.get("payload_json"))
    ...
    def _load_all_agents(self) -> list[Agent]:
        with self._connection.cursor() as cursor:
            cursor.execute("SELECT payload_json FROM agents ORDER BY created_at ASC")
            rows = cursor.fetchall()
        items: list[Agent] = []
        for row in rows:
            payload = self._deserialize_payload(row.get("payload_json"))
            if payload is not None:
                items.append(Agent.model_validate(payload))
        return items

    def _load_all_tasks(self) -> list[Task]:
        with self._connection.cursor() as cursor:
            cursor.execute("SELECT payload_json FROM tasks ORDER BY updated_at ASC")
            rows = cursor.fetchall()
        items: list[Task] = []
        for row in rows:
            payload = self._deserialize_payload(row.get("payload_json"))
            if payload is not None:
                items.append(Task.model_validate(payload))
        return items
